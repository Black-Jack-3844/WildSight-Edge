...
from torchvision import models  # 仅用 resnet18 结构（不使用 torchvision 的数据/变换）
...
def to_tensor_normalize(img: Image.Image) -> torch.Tensor:
    # PIL -> RGB -> float32/255 -> normalize -> CHW -> tensor
    img = img.convert("RGB")
...
def main():
    ...
    ap.add_argument("--workers", type=int, default=0)  # 保留参数占位，不使用
    ...
    ds_train = SimpleImageFolder(train_dir, train=True)
    classes = ds_train.classes
    ds_val = SimpleImageFolder(val_dir, train=False) if os.path.isdir(val_dir) else None
    ...
    print("Saved:", os.path.join(args.model_dir, "resnet18.pth"))
